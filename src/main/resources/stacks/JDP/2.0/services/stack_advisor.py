...
class JDP20StackAdvisor(DefaultStackAdvisor):

  def getComponentLayoutValidations(self, services, hosts):
    """Returns array of Validation objects about issues with hostnames components assigned to"""
    items = super(JDP20StackAdvisor, self).getComponentLayoutValidations(services, hosts)

    # Use a set for fast lookup
    hostsSet =  set(super(JDP20StackAdvisor, self).getActiveHosts([host["Hosts"] for host in hosts["items"]]))  #[host["Hosts"]["host_name"] for host in hosts["items"]]
  ...
  def recommendRangerConfigurations(self, configurations, clusterData, services, hosts):
    ...
    # Check if http is disabled. For JDP this can be checked in ranger-admin-site/ranger.service.http.enabled
