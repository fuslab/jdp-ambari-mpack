...
def setup_stack_symlinks():
  ...
  installed to ensure that all JDP pointers are correct. The stack upgrade logic does not
...
def link_configs(struct_out_file):
  """
  Links configs, only on a fresh install of JDP-0.3 and higher
  """
  import params

  if not Script.is_stack_greater_or_equal("0.3"):
    Logger.info("Can only link configs for JDP-0.3 and higher.")
    return
